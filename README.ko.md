...
**언어** — 한국어 · English · 日本語 · 简体中文 · Español · Français · Deutsch. 시스템 언어를 따라갑니다. 소스 문자열이 영어라 번역이 없는 언어는 영어로 떨어지고, 언어를 하나 더하는 일은 `Resources/<코드>.lproj/` 폴더 하나를 추가하는 것뿐입니다.
...
|---|---|---|
| 유휴 CPU | 4.2% | **0.05%** |
| 메모리 · 유휴, 재생 없음 | 57MB | **14MB** |
| 메모리 · 유휴, 재생 중 | — | **23~30MB** |
| 스레드 (유휴) | — | **3** |
| 확대/축소 중 CPU (프레임당) | — | **−25%** |
| 확대/축소 200회당 증가분 | +21MB | **0MB** |

숫자보다 조건이 중요해서 같이 적어 둡니다. 유휴 CPU는 **31.8시간 연속 실사용**(재생·패널·선반 전부 포함) 동안 쌓인 누적 CPU 54.2초를 나눈 값입니다 — 조용한 순간만 골라 잰 게 아닙니다.

메모리는 앨범아트가 올라와 있느냐가 거의 전부라 두 상태를 나눠 적었습니다. 재생 중에는 **같은 빌드에서 100초짜리 측정을 반복해도 23~30MB 사이**로 흔들리므로, 단일 측정값은 ±4MB로 보셔야 합니다. 31.8시간 실사용 뒤에는 49MB / 스레드 6개였습니다. 늘긴 하지만 **한 번 올라가고 멈춥니다** — 아래 ⑤가 그 얘기입니다.
